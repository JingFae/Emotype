...
| `GEMINI_API_KEY` | Google AI Studio key，用于图片上传的 Gemini VLM 分析，缺失时图片仅上传不分析。 |
...
完成部署配置或文档变更后，应运行仓库约定的 key 扫描命令。只出现 `os.getenv(...)`、`DEEPSEEK_API_KEY=...`、`GEMINI_API_KEY=...` 或占位符示例可以接受。
