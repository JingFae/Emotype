...
try:
    from emotion_rec.core.config import DEFAULT_DATABASE_PATH
except ModuleNotFoundError:  # Support ``uvicorn app:app`` from emotion_rec/.
    from core.config import DEFAULT_DATABASE_PATH  # type: ignore
...
class User(Base):
    ...
    participant_id: Mapped[int | None] = mapped_column(
        ForeignKey("participants.id"), unique=True, nullable=True, index=True,
    )
...
class UserSettings(Base):
    ...
    language: Mapped[str] = mapped_column(String(8), default="zh-CN")
    theme: Mapped[str] = mapped_column(String(32), default="warm")
    preferences_json: Mapped[Any] = mapped_column(JSON, default=dict)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: now_utc())


# ---------------------------------------------------------------------------
# Emo Echo chat models
# ---------------------------------------------------------------------------

class EchoSession(Base):
    __tablename__ = "echo_sessions"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    participant_id: Mapped[int] = mapped_column(ForeignKey("participants.id"), index=True)
    session_uuid: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: now_utc())
    last_active_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: now_utc())
...
class EchoMessage(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    session_id: Mapped[int] = mapped_column(ForeignKey("echo_sessions.id"), index=True)
...
def init_database() -> None:
    ...
    _ensure_sqlite_user_schema()
    _ensure_sqlite_echo_schema()
...
def _ensure_sqlite_user_schema() -> None:
    """Ensure user-related tables have all required columns in SQLite."""
    if engine.dialect.name != "sqlite":
        return
...
def delete_all_participant_data(participant_code: str) -> dict:
    """Delete all diary_entries, formal_diaries, and emotion_review_reports for a participant."""
    with SessionLocal() as session:
        participant = _participant_by_code(session, participant_code)
        if participant is None:
            return {"deleted": 0, "diary_entries": 0, "formal_diaries": 0, "review_reports": 0}
        diary_count = session.execute(
            delete(DiaryEntry).where(DiaryEntry.participant_id == participant.id)
        ).rowcount
        formal_count = session.execute(
            delete(FormalDiary).where(FormalDiary.participant_id == participant.id)
        ).rowcount
        review_count = session.execute(
            delete(EmotionReviewReport).where(EmotionReviewReport.participant_id == participant.id)
        ).rowcount
        session.commit()
        return {"deleted": diary_count + formal_count + review_count, "diary_entries": diary_count, "formal_diaries": formal_count, "review_reports": review_count}
...
def _build_review_stats(
    ...
    emotion_label_colors: dict[str, str] = {}
    ...
    for record in records:
        ...
        if label:
            ...
            if label not in emotion_label_colors:
                ec = _normalize_hex_color(record.get("emotion_color"))
                if ec:
                    emotion_label_colors[label] = ec
    ...
    stats.update({
        ...
        "primary_emotions": _review_counter_items(primary_counter, 12, emotion_label_colors),
...
def _usage_event_review_record(event: UsageEvent) -> dict[str, Any] | None:
    ...
    if event_type == "body_sensation_advice":
        ...
        body_signals = _dedupe_texts(symptom_labels, 12)
        return {
            ...
            "fine_emotions": _dedupe_texts(symptom_labels, 12),
...
def _review_counter_items(counter: Counter[str], limit: int, color_map: dict[str, str] | None = None) -> list[dict[str, Any]]:
    return [
        {"label": label, "count": count, "color": (color_map or {}).get(label)}
...
def _verify_password(plain_password: str, hashed_password: str) -> bool:
    ...
    try:
        ...
        if hashed_password.startswith("sha256$"):
            ...
            return hashlib.sha256(f"{salt}:{plain_password}".encode()).hexdigest() == stored_hash
...
def create_user(
    ...
    if not USERNAME_RE.match(username):
        raise ValueError("用户名须为2-64位字母、数字、下划线或短横线。")
    if len(password) < 4:
    ...
    with SessionLocal() as session:
        ...
        # Create participant row for this user (participant_code = username)
        participant = _participant_by_code(session, username)
        if participant is None:
            participant = Participant(
...
def update_user_display_name(user_id: int, display_name: str) -> dict[str, Any]:
    with SessionLocal() as session:
        user = session.execute(
            select(User).where(User.id == user_id)
        ).scalar_one_or_none()
        if user is None:
            raise ValueError("用户不存在。")
        user.display_name = str(display_name or "").strip()[:128] or user.username
        session.commit()
        session.refresh(user)
        return user_to_dict(user)
...
def get_or_create_user_settings(user_id: int) -> dict[str, Any]:
    with SessionLocal() as session:
        ...
        return user_settings_to_dict(settings)
...
def update_user_settings(user_id: int, **kwargs: Any) -> dict[str, Any]:
    with SessionLocal() as session:
        ...
        if "language" in kwargs:
            settings.language = str(kwargs["language"])[:8]
        if "theme" in kwargs:
            settings.theme = str(kwargs["theme"])[:32]
        if "preferences_json" in kwargs:
            settings.preferences_json = safe_json(kwargs["preferences_json"] or {})
...
def user_to_dict(user: User) -> dict[str, Any]:
    """Serialize a User to dict (no password hash)."""
    return {
        "id": user.id,
...
def user_settings_to_dict(settings: UserSettings) -> dict[str, Any]:
    return {
        ...
        "preferences_json": settings.preferences_json or {},
        "updated_at": isoformat(settings.updated_at),
    }


# ---------------------------------------------------------------------------
# Emo Echo — SQLite migration
# ---------------------------------------------------------------------------

def _ensure_sqlite_echo_schema() -> None:
    if engine.dialect.name != "sqlite":
        return
    inspector = sa_inspect(engine)
    table_names = set(inspector.get_table_names())
    with engine.begin() as conn:
        if "echo_sessions" not in table_names:
            conn.execute(text(
        ...
        if "echo_messages" not in table_names:
            conn.execute(text(
                "CREATE TABLE IF NOT EXISTS echo_messages ("
                "id INTEGER PRIMARY KEY, "
                "session_id INTEGER NOT NULL REFERENCES echo_sessions(id), "
...
# ---------------------------------------------------------------------------
# Emo Echo — session & message management
# ---------------------------------------------------------------------------

def get_or_create_echo_session(participant_code: str, session_uuid: str) -> dict[str, Any]:
    code = normalize_participant_code(participant_code or "local")
    with SessionLocal() as session:
        participant = _get_or_create_participant_inner(session, code, "research-v1")
        existing = session.execute(
            select(EchoSession).where(EchoSession.session_uuid == session_uuid)
        ).scalar_one_or_none()
        if existing:
            existing.last_active_at = now_utc()
            session.commit()
            session.refresh(existing)
            return {"id": existing.id, "session_uuid": existing.session_uuid}
        new_session = EchoSession(
            participant_id=participant.id,
...
def save_echo_message(session_db_id: int, role: str, content: str) -> None:
    with SessionLocal() as session:
        msg = EchoMessage(session_id=session_db_id, role=role, content=content)
        session.add(msg)
        session.commit()


def list_echo_messages(session_db_id: int, limit: int = 50) -> list[dict[str, Any]]:
    with SessionLocal() as session:
        rows = session.execute(
            select(EchoMessage)
            .where(EchoMessage.session_id == session_db_id)
            .order_by(EchoMessage.created_at.desc())
            .limit(limit)
        ).scalars().all()
        return [
            {"role": m.role, "content": m.content, "created_at": isoformat(m.created_at)}
            for m in reversed(rows)
        ]


def _get_or_create_participant_inner(session: Session, code: str, consent_version: str) -> Participant:
...
# ---------------------------------------------------------------------------
# Emo Echo — build user context string for LLM
# ---------------------------------------------------------------------------

def list_echo_sessions(participant_code: str, limit: int = 30) -> list[dict[str, Any]]:
    """List recent echo sessions with their first/last message previews."""
    code = normalize_participant_code(participant_code or "local")
    with SessionLocal() as session:
        participant = _participant_by_code(session, code)
        if participant is None:
        ...
        sessions = session.execute(
            select(EchoSession)
            .where(EchoSession.participant_id == participant.id)
            .order_by(EchoSession.last_active_at.desc())
            .limit(limit)
        ).scalars().all()
        result = []
        for s in sessions:
            msgs = session.execute(
                select(EchoMessage)
                .where(EchoMessage.session_id == s.id)
                .order_by(EchoMessage.created_at.asc())
                .limit(50)
            ).scalars().all()
            first_user = next((m.content[:60] for m in msgs if m.role == "user"), "")
            result.append({
                "id": s.id,
                "session_uuid": s.session_uuid,
                "created_at": isoformat(s.created_at),
                "last_active_at": isoformat(s.last_active_at),
                "message_count": len(msgs),
                "preview": first_user,
                "messages": [{"role": m.role, "content": m.content, "created_at": isoformat(m.created_at)} for m in msgs],
            })
        return result


def get_echo_context(participant_code: str, days: int = 15) -> str:
    """Build a compact text summary of the user's recent records for LLM context."""
    ...
    with SessionLocal() as session:
        ...
        diaries = session.execute(
            select(FormalDiary)
            .where(
                FormalDiary.participant_id == participant.id,
                FormalDiary.diary_date >= start_str,
                FormalDiary.diary_date <= end_str,
                FormalDiary.is_draft == False,
            )
            .order_by(FormalDiary.diary_date.desc())
            .limit(10)
        ).scalars().all()
        ...
        if diaries:
            lines.append("【最近日记摘要】")
            for d in diaries:
                emotion = d.primary_emotion or ""
                preview = (d.content or "")[:100].replace("\n", " ")
                lines.append(f"- {d.diary_date} 情绪:{emotion} 内容:{preview}…")

        # --- Diary entries (随手记) ---
        cutoff_dt = datetime(end.year, end.month, end.day, 23, 59, 59, tzinfo=timezone.utc) - timedelta(days=days)
        entries = session.execute(
            select(DiaryEntry)
            .where(
                DiaryEntry.participant_id == participant.id,
                DiaryEntry.created_at >= cutoff_dt,
            )
            .order_by(DiaryEntry.created_at.desc())
            .limit(20)
        ).scalars().all()
        ...
        if entries:
            lines.append("【最近随手记】")
            for e in entries:
                label = e.final_label or e.original_label or ""
                text = (e.raw_text or e.transcript_text or "")[:80].replace("\n", " ")
                date_str = isoformat(e.created_at)[:10] if e.created_at else ""
                lines.append(f"- {date_str} 情绪:{label} 内容:{text}…")

        # --- Body sensation advice events (身体感受) ---
        body_events = session.execute(
            select(UsageEvent)
            .where(
                UsageEvent.participant_id == participant.id,
                UsageEvent.event_type == "body_sensation_advice",
                UsageEvent.created_at >= cutoff_dt,
            )
        ...
        if body_events:
            ...
            for ev in body_events:
                ...
                symptoms = [str(s.get("label") or s.get("id") or "") for s in (meta.get("symptoms") or []) if isinstance(s, dict)]
                regions = [str(r.get("label") or r.get("id") or "") for r in (meta.get("selected_regions") or []) if isinstance(r, dict)]
                date_str = isoformat(ev.created_at)[:10] if ev.created_at else ""
                parts = []
                if regions:
                    parts.append("部位:" + "、".join(regions))
                if symptoms:
                    parts.append("感受:" + "、".join(symptoms))
