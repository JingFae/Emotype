"""FastAPI application assembly and backward-compatible route implementations."""
...
from fastapi import FastAPI, UploadFile, File, Form, Header, Query, HTTPException, Depends
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import Response
from fastapi.staticfiles import StaticFiles

try:
    from emotion_rec.api.schemas import (
        BodySensationAdviceRequest,
        CombinedAnalysisRequest,
        DiaryEntryRequest,
        DiaryReflectRequest,
        EchoChatRequest,
        FormalDiaryUpsertRequest,
        LoginRequest,
        ParticipantSessionRequest,
        PasswordChangeRequest,
        ProfileUpdateRequest,
        RegisterRequest,
        ReviewReflectRequest,
        SettingsUpdateRequest,
        TextAnalysisRequest,
        UsageEventRequest,
    )
    from emotion_rec.api.routes.pages import router as page_router
    from emotion_rec.core.config import (
        SHARED_DIR,
        STATIC_DIR,
    )
    from emotion_rec.core.security import create_access_token, verify_token
    from emotion_rec.domain.emotion.mapping import (
        EMOTION_LEXICON,
        map_segments,
        map_va,
        normalize_vad,
    ...
    from emotion_rec.domain.emotion.text import analyze_text_emotion
    from emotion_rec.integrations import llm as llm_client
    from emotion_rec.integrations import vision as gemini_client
    from emotion_rec.persistence.repository import (
        create_diary_entry,
        create_user,
        authenticate_user,
    ...
    from emotion_rec.services import audio_emotion
    ...
    from api.routes.pages import router as page_router  # type: ignore
    from core.config import SHARED_DIR, STATIC_DIR  # type: ignore
    from core.security import create_access_token, verify_token  # type: ignore
    from domain.emotion.mapping import (  # type: ignore
        EMOTION_LEXICON,
        map_segments,
        map_va,
        normalize_vad,
    ...
    from domain.emotion.text import analyze_text_emotion  # type: ignore
    from integrations import llm as llm_client  # type: ignore
    from integrations import vision as gemini_client  # type: ignore
    from persistence.repository import (  # type: ignore
        create_diary_entry,
        create_user,
        authenticate_user,
        get_user_by_username,
    ...
    from services import audio_emotion  # type: ignore
...
# Keep private aliases while routes migrate to the core security vocabulary.
...
# Public aliases retained for scripts that previously imported audio helpers
# from emotion_rec.app. New code should import services.audio_emotion directly.
RegressionHead = audio_emotion.RegressionHead
EmotionModel = audio_emotion.EmotionModel
MODEL_NAME_OR_PATH = audio_emotion.MODEL_NAME_OR_PATH
TARGET_SR = audio_emotion.TARGET_SAMPLE_RATE
device = audio_emotion.DEVICE
convert_webm_to_wav = audio_emotion.convert_to_wav
extract_acoustic_features = audio_emotion.extract_acoustic_features
...
def healthz():
    return {
        "status": "ok",
        "device": str(audio_emotion.DEVICE),
        "model_loaded": audio_emotion.model_loaded(),
...
async def body_sensation_advice(payload: BodySensationAdviceRequest):
    try:
        from emotion_rec.services.body_sensation import generate_body_sensation_advice
    except ModuleNotFoundError:
        from services.body_sensation import generate_body_sensation_advice  # type: ignore
...
async def predict(
    ...
    try:
        ...
        # 1. 音频处理
        try:
            wav_arr, wav_path = audio_emotion.read_audio_to_mono_16k(raw)
            vad_scores, embeddings_vec = audio_emotion.predict_raw_vad(wav_arr)
        except audio_emotion.AudioProcessingError as error:
            raise HTTPException(status_code=400, detail=str(error)) from error
        except audio_emotion.ModelUnavailableError as error:
        ...
        acoustics = audio_emotion.extract_acoustic_features(wav_path)
        segment_inputs = [
            {
                "text": segment,
