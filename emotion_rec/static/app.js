...
const I18N = {
  zh: {
    "brand.tag": "情绪镜像日记",
    "nav.home": "首页", "nav.journal": "日记", "nav.body": "身体感受", "nav.data": "数据",
    "topbar.local": "本地模式",
  ...
  en: {
    ...
    "nav.home": "Home", "nav.journal": "Journal", "nav.body": "Body Sense", "nav.data": "Data",
...
function initI18n() {
  ...
  btn.addEventListener("click", () => {
    ...
    localStorage.setItem("emomirror.lang", currentLang);
    applyI18n();
    renderHome();
    renderBodyChips();
  });
...
function bodyParticipantCode() {
  ...
  try { return (input && input.value.trim()) || localStorage.getItem("emomirror.participant") && JSON.parse(localStorage.getItem("emomirror.participant"))?.participant_code || ""; }
  catch (e) { return ""; }
}
...
function initBodyTab() {
  ...
  if (clearEntriesData) clearEntriesData.addEventListener("click", () => {
    entries = [];
    saveEntries();
    renderHome();
  });
